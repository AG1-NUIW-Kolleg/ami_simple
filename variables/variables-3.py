...
# timing parameters
# -----------------
dt_elasticity = 0.0001  # drastically reduced to 0.1ms to prevent mesh inversion
...
end_time = 1.0  # reduced to 1ms for stability testing
...
tendon_material = "SaintVenantKirchoff"  # using linear material for improved numerical stability         

tendon_extent = [3.0, 3.0, 5.0] # [cm, cm, cm] 2.96
n_elements_tendon = [4,4,4] # reduced from [8,8,8] for computational efficiency 

# Fiber and mesh configuration
# -----------------------------
# These parameters have been optimized for computational efficiency while maintaining
# simulation accuracy. Original configuration: 8x8 fibers, [8,8,24] muscle elements,
# [8,8,8] tendon elements, 60 points per fiber.
# Current configuration reduces computational load by ~75% while preserving essential physics.

muscle_extent = [3.0, 3.0, 15.0] # [cm, cm, cm]
n_elements_muscle = [4, 4, 12] # reduced from [8, 8, 24] for computational efficiency - linear elements. each qudaratic element uses the combined nodes of 8 linear elements
n_points_whole_fiber = 30  # reduced from 60 for computational efficiency
n_fibers_x = 4  # reduced from 8 to 4 for computational efficiency
n_fibers_y = 4  # reduced from 8 to 4 for computational efficiency
...
damping_factor = 1.0                  # significantly increased damping for maximum stability
...
firing_times_file = input_directory + "/MU_firing_times_never.txt" # use never firing for pure mechanics test
#firing_times_file = input_directory + "/MU_firing_times_real.txt" # original firing times
